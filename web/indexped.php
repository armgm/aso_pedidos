...
//Aqui comienza la meta
		...
		echo "<td colspan=\"5\" align=\"center\">Meta del Vendedor</td> \n";
		echo "</tr>\n";
		echo "<tr>\n";		
...
//Aqui comienza el detelle de los pedidos		
		$sql="select a.nropedido,a.fecha_ped,b.nombre from pedidos as a, clientes as b where trim(a.zona) = '$zona' 			and a.codcli=b.codcli and a.fecha_ped between '".fecha_des($fecha)."' and '".fecha_has($fecha)."' order by 			a.fecha_ped";
	//	echo $sql;
		$result=mysql_query($sql,$link);
		echo "<table cellpadding=\"3px\" cellspacing=\"0px\" border = \"1\"> \n";
		echo "<tr  bgcolor=\"#6699CC\" align=\"center\"><td colspan=\"5\">Detalle Pedido/Facturas</td><tr> \n";		
		echo "<tr> \n";
		...
		while ($row = mysql_fetch_row($result)){
			...
			{
				echo "<td><a href=\"detfac.php?fac=$factura&zona=$zona&fecha=$fecha&vend=$vend&cor=$cord\">$factura</a></td> \n";}
			else{
				echo "<td align=\"center\">$factura</td> \n";}
			echo "<td>".busca_fec_fac($pedido,$link)."</td> \n";
			echo "<td>$row[2]</td> \n";
			echo "</tr> \n";
			}
		echo "</table> \n";
